...
var ajaxifyShopify = (function(module, $) {
  ...
  flipSetup = function () {
    ...
    var checkoutBtn = $('<a href="' + cartUrl + '" class="flip-back" style="background-color: #C00; color: #fff;" id="flip-checkout">' + "Zur Kasse" + '</a>').addClass($btnClass),
  ...
  buildCart = function (cart) {
    ...
    // Show empty cart
    if (cart.item_count === 0) {
      $cartContainer.append('<h2>' + "Dein Warenkorb ist im Moment leer." + '</h2><span class="cart--continue-message">' + "Mit der Suche \u003ca href=\"\"\u003ehier\u003c\/a\u003e fortfahren." + '</span><span class="cart--cookie-message">' + "Aktiviere Cookies, um den Warenkorb benutzen zu können" + '</span>');
    ...
    productVariantCallback: function(variant) {
      ...
      if (variant) {
        ...
        if (variant.available) {
          ...
          this.settings.selectors.$addToCartText.html("In den Warenkorb legen");
          ...
          /***CUSTOM CODE -- Lieferzeit***/
          
          //check if inventory is managed
          if(variant.inventory_management == 'shopify') {
            
            let customcode_inventory = undefined;
            
            //get metadata-<div>'s from html page
            const variantInvMetaElements = document.querySelectorAll('.inventory_metadata');
            variantInvMetaElements.forEach(function(variantElm) {
              
              //search for correct metadata-<div> and save inventory-data to variable
              if(variantElm.dataset.variantId == variant.id) {
                customcode_inventory = variantElm.dataset.variantInventoryQty;
              }
            });
            
            //FOR DEBUG PURPOSES: Output inventory
            //console.debug("Inventory: " + customcode_inventory);
            
            //place correct inventory code on html
            if(customcode_inventory > 0) {
              //product is in stock
              document.getElementById('variant-inventory').innerHTML = '<svg style="vertical-align: middle;" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#0F7A18"><path d="M0 0h24v24H0z" fill="none"/><path d="M20 8h-3V4H3c-1.1 0-2 .9-2 2v11h2c0 1.66 1.34 3 3 3s3-1.34 3-3h6c0 1.66 1.34 3 3 3s3-1.34 3-3h2v-5l-3-4zM6 18.5c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zm13.5-9l1.96 2.5H17V9.5h2.5zm-1.5 9c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"/></svg><span style="line-height: 32px; padding-left: 5px; color: #0F7A18; font-weight: bold;">Lieferzeit: ca. 2-3 Tage</span><br><span style="color: #0F7A18; font-size: 11px;">Bei größeren Bestellmengen kann die Lieferzeit abweichen.</span>';
            } else {
              //product is not in stock
              document.getElementById('variant-inventory').innerHTML = '<svg style="vertical-align: middle;" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#831D12"><path d="M0 0h24v24H0z" fill="none"/><path d="M20 8h-3V4H3c-1.1 0-2 .9-2 2v11h2c0 1.66 1.34 3 3 3s3-1.34 3-3h6c0 1.66 1.34 3 3 3s3-1.34 3-3h2v-5l-3-4zM6 18.5c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zm13.5-9l1.96 2.5H17V9.5h2.5zm-1.5 9c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"/></svg><span style="line-height: 32px; padding-left: 5px; color: #831D12; font-weight: bold;">Lieferzeit: ca. 2-3 Wochen </span><br><span style="color: #831D12;">Dieses Produkt ist derzeit nicht lagernd.<br>Wir versenden dein Paket sobald das Produkt bei uns ankommt!</span>';
            }
            
          } else {
            //inventory isn't tracked by shopify
            //display nothing
            document.getElementById('variant-inventory').innerHTML = '';
          }
          /***CUSTOM CODE END***/
        ...
        if ( variant.compare_at_price > variant.price ) {
          ...
          this.settings.selectors.$comparePrice.html("Du sparst [$]".replace('[$]', priceSaving)).show();
        } else {
